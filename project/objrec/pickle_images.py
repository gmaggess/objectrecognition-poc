...
on the image name and classes mapping, class_mapping, and saves them into pickle file, data_file"""
    logging.info('Data File: ' + str(data_file))
    # the training data
    X = []
    # the taining labels
    Y = []
    ...
    logging.debug("serializing image arrays to " + data_file)
    ...
    logging.debug("reading arrays from pickle file " + data_file)
    f = open(data_file, "rb")
    A, B, C, D = pickle.load(f)

    logging.debug(A.shape)
    logging.debug(B.shape)
