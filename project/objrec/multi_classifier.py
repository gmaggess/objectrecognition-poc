...
def cnn_model_fn(features, labels, mode):
    ...
    input_layer = tf.reshape(
        features["x"], [-1, FLAGS.img_w, FLAGS.img_h, FLAGS.img_channels])
    ...
    onehot_labels = tf.one_hot(indices=tf.cast(
        labels, tf.int32), depth=FLAGS.classes)
...
def main(unused_argv):
    ...
    if FLAGS.mode == tf.estimator.ModeKeys.EVAL:
        train_data, train_labels, eval_data, eval_labels = pickle.load(
            open(FLAGS.data_set, "rb"))
...
if __name__ == "__main__":
    ...
    if FLAGS.mode == tf.estimator.ModeKeys.TRAIN or FLAGS.mode == tf.estimator.ModeKeys.EVAL:
        if FLAGS.data_set is None:
            parser.error(
                "--data_set must be provided if --mode is train or eval")
    tf.app.run()
